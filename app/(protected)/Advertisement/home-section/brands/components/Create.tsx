...
import { uploadBrandsImage } from "@/app/(protected)/services/brands/api";
...
const CreateModal: React.FC<CreateModalProps> = ({
  ...
  const handleSubmit = async () => {
    try {
      const result = await uploadBrandsImage({
        name: formData.Name,
      ...
      if (result.data) {
        toast.success("Brand image added");
        console.log("Upload success:", result.data);
        setMobileFile(null);
        refreshData();
        handleClose();
      } else {
        toast.error("Brand image not added");
      }
    } catch (error) {
